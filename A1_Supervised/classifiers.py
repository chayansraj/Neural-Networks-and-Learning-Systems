...
def kNN(X, k, XTrain, LTrain):
    ...
    for i in range(0, X.shape[0]):
        ...
        for j in range(XTrain.shape[0]):
            arr[j] = np.sqrt(np.sum((X[i]-XTrain[j])**2)) 
        if X is XTrain:
            if k == 1:
...
def trainMultiLayer(XTrain, DTrain, XTest, DTest, W0, V0, numIterations, learningRate):
    ...
    for n in range(numIterations):
        ...
        # Add your own code here
        grad_v = 2*np.dot(HTrain.transpose(),(YTrain - DTrain))/NTrain      # Gradient for the output layer
        grad_w = 2*np.matmul(XTrain.transpose(),(np.multiply(np.matmul(YTrain-DTrain,Vout.transpose()),(1-HTrain**2))))/NTrain    # And the input layer
